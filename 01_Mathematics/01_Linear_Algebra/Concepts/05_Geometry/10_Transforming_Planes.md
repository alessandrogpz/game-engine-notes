...
$$
d^B = -\left( \mathbf{n}^A \mathrm{adj}(\mathbf{M}) \right) \cdot \mathbf{q}^B
$$

$$
d^B = -\mathbf{n}^A \mathrm{adj}(\mathbf{M}) \cdot \mathbf{H}\mathbf{q}^A
$$

Expanding $\mathbf{H}\mathbf{q}^A$ into its rotation-scale and translation parts:

$$
d^B = -\mathbf{n}^A \mathrm{adj}(\mathbf{M}) \cdot \left( \mathbf{M}\mathbf{q}^A + \mathbf{t} \right)
$$

$$
d^B = -\mathbf{n}^A \det(\mathbf{M})\mathbf{q}^A - \mathbf{n}^A \mathrm{adj}(\mathbf{M})\mathbf{t}
$$
...
The first term collapses because the adjugate satisfies $\mathrm{adj}(\mathbf{M})\mathbf{M} = \det(\mathbf{M})\mathbf{I}$, and the second because $\mathrm{adj}(\mathbf{M}) = \det(\mathbf{M})\mathbf{M}^{-1}$ lets $\det(\mathbf{M})$ be factored out of both terms. Recognising $-\mathbf{n}^A \cdot \mathbf{q}^A$ as $d^A$ completes it.
...
$$
\mathbf{f}^B = \mathbf{f}^A \det(\mathbf{H})\mathbf{H}^{-1} = \mathbf{f}^A \mathrm{adj}(\mathbf{H})
$$

> [!IMPORTANT]
> Note the **side** of the multiplication. A point is a column vector transformed from the left, $\mathbf{q}^B = \mathbf{H}\mathbf{q}^A$, while a plane is a **row vector transformed from the right** by a different matrix entirely. Planes are not objects that live in space the way points do — they are linear functionals *measuring* space, and that dual nature is exactly why they transform by $\mathrm{adj}(\mathbf{H})$ on the opposite side rather than by $\mathbf{H}$.
...
$$
\mathbf{n}^B = \mathbf{n}^A \det(\mathbf{M})\mathbf{M}^{-1} = \mathbf{n}^A \mathrm{adj}(\mathbf{M})
$$
...
* **C++ Source Code:** [Transforming_Planes.cppm](../../../../90_Code/05_Geometry/Transforming_Planes.cppm)
