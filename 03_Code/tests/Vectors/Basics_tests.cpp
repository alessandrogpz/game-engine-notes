...
TEST(Vector3Basics, ConstructorsAndComponents) {
    ...
    EXPECT_FLOAT_EQ(v1.z, 0.0);

    vectors::vector3 v2(1.5, -2.0, 3.1);
    EXPECT_FLOAT_EQ(v2.x, 1.5);
    EXPECT_FLOAT_EQ(v2.y, -2.0);
    EXPECT_FLOAT_EQ(v2.z, 3.1);
}
...
TEST(Vector3Basics, ScalarMultiplicationDivision) {
    ...
    EXPECT_FLOAT_EQ(prod.x, 5.0);
    EXPECT_FLOAT_EQ(prod.y, -10.0);
    EXPECT_FLOAT_EQ(prod.z, 20.0);

    vectors::vector3 quot = v / 2.0;
    EXPECT_FLOAT_EQ(quot.x, 1.0);
...
TEST(Vector3Basics, MagnitudeAndNormalization) {
    ...
    EXPECT_FLOAT_EQ(v_norm.x, 0.6);
    EXPECT_FLOAT_EQ(v_norm.y, 0.0);
    EXPECT_FLOAT_EQ(v_norm.z, 0.8);

    // Zero vector magnitude and normalization
    vectors::vector3 zero;
    EXPECT_FLOAT_EQ(zero.magnitude(), 0.0);
    vectors::vector3 zero_norm = zero.normalized();
    EXPECT_FLOAT_EQ(zero_norm.x, 0.0);
    EXPECT_FLOAT_EQ(zero_norm.y, 0.0);
    EXPECT_FLOAT_EQ(zero_norm.z, 0.0);
}

TEST(Vector4Basics, ConstructorsAndArithmetic) {
    vectors::vector4 v1(1.0, 2.0, 3.0, 4.0);
    EXPECT_FLOAT_EQ(v1.x, 1.0);
    EXPECT_FLOAT_EQ(v1.w, 4.0);

    vectors::vector3 v3(5.0, 6.0, 7.0);
    vectors::vector4 v4(v3, 2.0);
    EXPECT_FLOAT_EQ(v4.x, 5.0);
    EXPECT_FLOAT_EQ(v4.w, 2.0);

    vectors::vector4 sum = v1 + v4;
    EXPECT_FLOAT_EQ(sum.x, 6.0);
    EXPECT_FLOAT_EQ(sum.w, 6.0);

    vectors::vector4 prod = v1 * 2.0;
    EXPECT_FLOAT_EQ(prod.w, 8.0);
}
...
TEST(MatrixVectorMultiplication, AffinePointTransform) {
    ...
    EXPECT_FLOAT_EQ(p_trans.x, 12.0);
    EXPECT_FLOAT_EQ(p_trans.y, -2.0);
    EXPECT_FLOAT_EQ(p_trans.z, 4.0);
}
