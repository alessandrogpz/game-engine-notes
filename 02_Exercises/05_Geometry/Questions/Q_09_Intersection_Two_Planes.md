...
1. Two planes are given by $\mathbf{f}_1 = [1, 0, 0, -2]$ and $\mathbf{f}_2 = [0, 1, 0, -3]$:
   ...
   * Compute the two [[04_Cross_Product|cross products]] $\mathbf{v} \times \mathbf{n}_2$ and $\mathbf{n}_1 \times \mathbf{v}$.
   * Solve for the point $\mathbf{p}$ and state the full [[03_Lines_and_Rays|parametric line]] $L(t) = \mathbf{p} + t\mathbf{v}$.
...
3. Confirm the identity that makes the denominator collapse. For the planes in Question 2, compute the [[05_Scalar_Triple_Product|scalar triple product]] $[\mathbf{n}_1, \mathbf{n}_2, \mathbf{v}]$ directly from the [[03_Determinants|determinant]] of the $3 \times 3$ matrix whose rows are $\mathbf{n}_1$, $\mathbf{n}_2$ and $\mathbf{v}$, and show it equals $\mathbf{v}^2$.
...
---
**Check Answer:** [[S_09_Intersection_Two_Planes]] | **Related Concepts:** [[09_Intersection_Two_Planes]]
