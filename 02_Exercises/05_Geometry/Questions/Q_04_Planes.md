...
1. A parametric plane passes through starting position $\mathbf{p} = (1, 2, 3)$ with direction vectors $\mathbf{u} = \langle 1, 0, 2 \rangle$ and $\mathbf{v} = \langle 0, 3, 1 \rangle$:
   ...
   * Compute the [[02_Normal_Vectors|normal vector]] $\mathbf{n} = \mathbf{u} \times \mathbf{v}$.
...
2. A plane has a known point $\mathbf{q} = (2, -1, 4)$ lying on its surface and normal vector $\mathbf{n} = \langle 2, 3, -1 \rangle$:
   ...
   * Test whether the point $\mathbf{p}_1 = (1, 2, 2)$ lies on the plane by evaluating the 4D [[02_Dot_Product|dot product]] $\mathbf{f} \cdot \mathbf{p}_1$.

## Part 2: Conceptual Understanding
1. Explain why two non-parallel direction vectors $\mathbf{u}$ and $\mathbf{v}$ are required to represent a parametric plane, whereas a [[03_Lines_and_Rays|parametric line]] requires only a single direction vector.
