...
1. Explain why a plane's normal transforms by $\operatorname{adj}(\mathbf{M})$ rather than by $\mathbf{M}$ itself. Relate the row-vector form $\mathbf{n}^B = \mathbf{n}^A\operatorname{adj}(\mathbf{M})$ to the inverse-transpose rule for [normal vectors](../../../01_Concepts/05_Geometry/02_Normal_Vectors.md), and state why the two are the same statement.
2. In the derivation of $d^B$, the term $-\mathbf{n}^A\operatorname{adj}(\mathbf{M}) \cdot \mathbf{M}\mathbf{q}^A$ collapses to $-\mathbf{n}^A\det(\mathbf{M})\mathbf{q}^A$. Identify the [adjugate](../../../01_Concepts/03_Matrices/04_Matrix_Inversion.md) identity responsible, and show how the remaining translation term produces $-\mathbf{n}^A\mathbf{M}^{-1}\mathbf{t}$ after $\det(\mathbf{M})$ is factored out.
...
---
**Check Answer:** [S_10_Transforming_Planes](../Solutions/S_10_Transforming_Planes.md) | **Related Concepts:** [10_Transforming_Planes](../../../01_Concepts/05_Geometry/10_Transforming_Planes.md)
