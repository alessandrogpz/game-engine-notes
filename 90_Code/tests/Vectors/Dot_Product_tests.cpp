...
TEST(VectorsDotProduct, GeometricDot) {
    ...
    // 90 degrees = pi / 2
    EXPECT_NEAR(vectors::dotGeo(a, b, std::numbers::pi / 2.0), 0.0, 1e-5f);

    // Parallel case, 0 degrees
    EXPECT_FLOAT_EQ(vectors::dotGeo(a, a, 0.0), 4.0);
}

TEST(VectorsDotProduct, AngleBetween) {
    vectors::vector3 a(1.0, 0.0, 0.0);
    vectors::vector3 b(0.0, 1.0, 0.0);
    EXPECT_NEAR(vectors::angle(a, b), std::numbers::pi / 2.0, 1e-5f);

    // Collinear/Same direction
    EXPECT_NEAR(vectors::angle(a, a), 0.0, 1e-5f);

    // Zero vector safety
    vectors::vector3 zero;
    EXPECT_FLOAT_EQ(vectors::angle(a, zero), 0.0);
}
