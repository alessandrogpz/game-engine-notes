...
TEST(TransformsQuaternions, QuaternionBasics) {
    ...
    EXPECT_FLOAT_EQ(q1.w, 1.0);
    EXPECT_FLOAT_EQ(q1.x, 2.0);
    EXPECT_FLOAT_EQ(q1.y, 3.0);
    EXPECT_FLOAT_EQ(q1.z, 4.0);

    vectors::vector3 v(2.0, 3.0, 4.0);
    transforms::Quaternion q2(1.0, v);
    EXPECT_FLOAT_EQ(q2.w, 1.0);
    EXPECT_FLOAT_EQ(q2.x, 2.0);
    EXPECT_FLOAT_EQ(q2.y, 3.0);
    EXPECT_FLOAT_EQ(q2.z, 4.0);
}

TEST(TransformsQuaternions, MagnitudeConjugateInverse) {
    transforms::Quaternion q(1.0, 1.0, 1.0, 1.0);
    EXPECT_FLOAT_EQ(transforms::qMagnitude(q), 2.0); // sqrt(4) = 2

    transforms::Quaternion conj = transforms::qConjugate(q);
    EXPECT_FLOAT_EQ(conj.w, 1.0);
    EXPECT_FLOAT_EQ(conj.x, -1.0);

    transforms::Quaternion inv = transforms::qInverse(q);
    EXPECT_FLOAT_EQ(inv.w, 0.25);
    EXPECT_FLOAT_EQ(inv.x, -0.25);
}
...
TEST(TransformsQuaternions, VectorRotation) {
    ...
    EXPECT_NEAR(rot.x, -1.0, 1e-5f);
    EXPECT_NEAR(rot.y, 0.0, 1e-5f);
    EXPECT_NEAR(rot.z, 0.0, 1e-5f);
}
