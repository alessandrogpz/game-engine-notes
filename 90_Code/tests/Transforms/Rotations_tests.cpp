...
TEST(TransformsRotations, RotationX) {
    ...
    EXPECT_NEAR(res.x, 0.0, 1e-5f);
    EXPECT_NEAR(res.y, 0.0, 1e-5f);
    EXPECT_NEAR(res.z, 1.0, 1e-5f);
}
...
TEST(TransformsRotations, RotationZ) {
    ...
    EXPECT_NEAR(res.x, 0.0, 1e-5f);
    EXPECT_NEAR(res.y, 1.0, 1e-5f);
    EXPECT_NEAR(res.z, 0.0, 1e-5f);
}
